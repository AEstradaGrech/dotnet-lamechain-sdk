...
namespace Dotnet.OllamaSharp.LameChain.SDK.Command.Core.AtomicValues
{
    public class EnumPromptCommand<TEnum> : DbPromptCommand<TEnum> where TEnum : struct, Enum
    {
        ...
        public override async Task<TEnum> Prompt(PromptCommandRequest request)
        {
            ...
            var response = await _ollama.CommandPrompt<IntegerChoiceResponse>(request.ToOllamaChat(systemMessage, _settings), _settings == null ? null : validatorFor<IntegerChoiceResponse>(_settings.CommandValidations, _settings.ValidationType), request.Provider, GetRequestTools(request));
